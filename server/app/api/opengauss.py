...
class RatingManager:
    ...
    def add_rating(self, score,content):
        ...
        try:
            with conn.cursor() as cursor:
                sql = "INSERT INTO ratings (score,content) VALUES (%s,%s) RETURNING id"
                cursor.execute(sql, (score,content))
                result = cursor.fetchone()  # 现在可以正确获取到返回的id
                conn.commit()
    ...
    def add_video(self, name,annotation,size,url,duration,thumbnail):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                result = cursor.fetchone()  # 获取 RETURNING 的结果
                conn.commit()
                return result['id'] if result else -1  # 从字典中获取id
...
class TrainingPlanManager:
    ...
    def get_trained_date(self, user_id,year=None,month=None):
        ...
        if year is not None:
            keys.append(" AND EXTRACT(YEAR FROM date) = %s ")
            param.append(year)
        if month is not None:
            keys.append(" AND EXTRACT(MONTH FROM date) = %s ")
            param.append(month)
